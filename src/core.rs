...
pub(super) fn generate(
    input: String, qroptions: QROptions, logo: &[u8], image_size: usize,
    ...
    if qroptions.add_transparency == Some(true) {
        make_transparent(&mut qr);
    }
    if qroptions.add_logo {
        add_logo(&mut qr, logo);
    }
    if qroptions.add_gradient {
        add_gradient(&mut qr)
    }
    let mut result = vec![];
...
fn add_logo(qr: &mut ImageBuffer<Rgba<u8>, Vec<u8>>, logo: &[u8]) {
    let img_size = qr.width().min(qr.height()) as usize;
    let element_size = get_qr_element_size(qr);
    let mut logo_size = element_size;
    while logo_size + 2 * element_size <= 5 * img_size / 16 {
        logo_size += 2 * element_size;
    }
    let mut logo = image::io::Reader::new(Cursor::new(logo)).with_guessed_format()
        .unwrap()
        .decode()
        .unwrap();
    logo = logo.resize(
...
fn add_gradient(qr: &mut ImageBuffer<Rgba<u8>, Vec<u8>>){
    ...
    for (x,y,pixel) in qr.enumerate_pixels_mut(){
        if pixel.0 == [0,0,0,255] {
            ...
            let rgba = gradiant.at(distance as f64 /image_size as f64).to_rgba8();
...
fn get_qr_element_size(qr: &mut ImageBuffer<Rgba<u8>, Vec<u8>>) ->usize{
    ...
    for i in 0..size -start{
        if qr.get_pixel(start+i,start+1).0 != BLACK_PIXEL{
            element_size = i;
            break;
        }
    }
